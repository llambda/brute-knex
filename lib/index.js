        ...
        self.ready = self.knex.schema.hasTable(self.options.tablename).then(function (exists) {
            if (!exists) {
                return self.knex.schema.createTable(self.options.tablename, function (table) {
                    table.string('key');
                    table.timestamp('firstRequest').defaultTo(self.knex.fn.now());
                    table.timestamp('lastRequest').defaultTo(self.knex.fn.now());
                    table.timestamp('lifetime').defaultTo(self.knex.fn.now());
                    table.integer('count');
                })
            }
        })
